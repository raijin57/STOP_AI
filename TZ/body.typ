...
#listhead2[
Разработка программного продукта ведётся на основании анкеты для извлечения требований (опросного листа) от 20 октября 2025 г., предназначенной для выявления, фиксации и согласования требований к функционалу, ограничениям и условиям эксплуатации. Требования настоящего технического задания сформированы путём анализа ответов анкеты.
]
...
#h(0em)
#pad(left: 6.4em)[
- Личный данные пользователя такие как логин и пароль храняться на сервере, чтобы была возможность авторизовацаться 
- 
]
...
#h(0em)
#pad(left: 6.4em)[
- Приложение должно предоставлять пользователю настройки конфиденциальности данных действий (факты блокировок/попыток доступа/изменения правил).
- Приложение не должно собирать и хранить содержимое запросов пользователя (промпты) как часть MVP.
]

==== Хранение данных локально по умолчанию
#h(0em)
#pad(left: 6.4em)[
- Пользовательские настройки и правила должны храниться локально на устройстве по умолчанию.
- При отсутствии сети локальные механизмы блокировки и правила должны продолжать работать.
]

==== Аутентификация
#h(0em)
#pad(left: 6.4em)[
- Приложение должно поддерживать аутентификацию следующими способами:
  - через Google-аккаунт;
  - через номер телефона (SMS-код);
  - через логин и пароль.
- При успешной аутентификации должна создаваться пользовательская сессия (с сохранением состояния входа).
- При ошибке аутентификации приложение должно показывать причину и не менять текущий статус сессии.
]

==== Блокировка браузерного ИИ
#h(0em)
#pad(left: 6.4em)[
- Приложение должно предоставлять включение/выключение блокировки браузерного ИИ как отдельной категории.
- При активной блокировке приложение должно препятствовать доступу к ресурсам, подпадающим под правила блокировки, и фиксировать факт блокировки.
]

==== Блокировка ИИ-приложений
#h(0em)
#pad(left: 6.4em)[
- Приложение должно предоставлять включение/выключение блокировки ИИ-приложений как отдельной категории.
- Механизм блокировки должен использовать доступные системные средства на поддерживаемой платформе.
]

==== Блокировка ИИ-ботов
#h(0em)
#pad(left: 6.4em)[
- Приложение должно предоставлять включение/выключение блокировки ИИ-ботов как отдельной категории.
- Если объект не может быть заблокирован технически на конкретной платформе, приложение должно фиксировать ограничение и не вводить пользователя в заблуждение.
]

==== Персональные фильтры степени блокировки
#h(0em)
#pad(left: 6.4em)[
- Приложение должно поддерживать настройку персональных правил блокировки (фильтров) по категориям: браузерный ИИ / ИИ-приложения / ИИ-боты.
- Правило фильтра должно содержать: тип объекта, идентификатор (домен/пакет/бот), действие (запрет/разрешение в рамках исключений), уровень блокировки.
- Пользователь должен иметь возможность создавать, изменять, отключать и удалять фильтры, а также просматривать список активных правил.
]

==== Уровень блокировки
#h(0em)
#pad(left: 6.4em)[
- Приложение должно поддерживать параметр «уровень блокировки» (например: мягкий/средний/жёсткий).
- Уровень блокировки должен влиять на строгость применения фильтров и доступность временных исключений согласно заданной политике приложения.
]

==== Постепенное ужесточение блокировки
#h(0em)
#pad(left: 6.4em)[
- Приложение должно поддерживать режим «постепенного ужесточения» блокировки.
- При включенном режиме приложение должно автоматически снижать «допуски» при попытках обхода/временного снятия запрета (например: уменьшать доступное время разблокировки, увеличивать паузу между попытками, ограничивать число попыток).
- Пользователь должен иметь возможность включить режим, задать параметры и сохранить настройки.
]

==== Дневные лимиты
#h(0em)
#pad(left: 6.4em)[
- Приложение должно предоставлять настройку дневных лимитов для выбранных категорий и/или правил блокировки.
- При достижении лимита приложение должно применять заданное поведение (например, блокировать доступ до конца суток либо до момента, заданного политикой лимита).
- Пользователь должен иметь возможность проверить состояние лимита (активен/достигнут).
]

==== Белый список (исключения)
#h(0em)
#pad(left: 6.4em)[
- Приложение должно поддерживать белый список исключений для сайтов/приложений/ботов.
- Пользователь должен иметь возможность добавить исключение, указав тип объекта и идентификатор, а также удалить исключение.
]

==== Временное снятие запрета по таймеру
#h(0em)
#pad(left: 6.4em)[
- Приложение должно поддерживать временное снятие запрета по таймеру: пользователь задаёт длительность и подтверждает действие.
- По завершении таймера приложение должно автоматически восстановить действие блокировки.
]

==== Временное снятие запрета по расписанию
#h(0em)
#pad(left: 6.4em)[
- Приложение должно поддерживать временное снятие запрета по расписанию: пользователь задаёт дни недели и интервал времени и сохраняет правило.
- Приложение должно позволять просматривать, изменять, отключать и удалять правила снятия запрета по расписанию.
]

==== Установка запрета по таймеру
#h(0em)
#pad(left: 6.4em)[
- Приложение должно поддерживать «сеанс блокировки» по таймеру: пользователь задаёт длительность, выбирает категории блокировки и запускает таймер запрета.
- Во время активного таймера запрета приложение должно обеспечивать применение блокировки согласно выбранным категориям и фильтрам.
]

==== Установка запрета по расписанию
#h(0em)
#pad(left: 6.4em)[
- Приложение должно поддерживать расписание блокировки (регулярные интервалы, когда запрет активен) для выбранных категорий и/или наборов фильтров.
- Пользователь должен иметь возможность создать, изменить, отключить и удалить правило расписания блокировки.
- Пересечения расписаний должны обрабатываться корректно (объединение интервалов), локальное время трактуется одинаково на всех платформах.
]

==== Приоритеты правил при конфликте
#h(0em)
#pad(left: 6.4em)[
- Приложение должно определять приоритеты правил при конфликте и применять их одинаково на всех платформах.
- Для целей MVP приоритеты фиксируются следующим образом (от более высокого к более низкому):
  1) активный таймер запрета;
  2) активное расписание запрета;
  3) достигнутый дневной лимит;
  4) персональные фильтры и выбранный уровень блокировки;
  5) белый список (исключения);
  6) активное окно временного снятия запрета (таймер/расписание снятия запрета).
]
...
- Импортируемые данные должны валидироваться; при ошибке импорт не должен применяться частично.
- При успешном импорте приложение должно уведомить о применении настроек.
]
...
- Экспорт должен включать: категории блокировки, фильтры, уровень блокировки, параметры ужесточения, лимиты, белый список, таймеры/расписания.
]

==== Управление подпиской и платный доступ
#h(0em)
#pad(left: 6.4em)[
- Приложение должно поддерживать механизм подписки (оформление/изменение/отмена) и отображение статуса подписки.
- Приложение должно разграничивать доступ к функциям на основании статуса подписки.
]

==== Платформенная совместимость (MVP)
#h(0em)
#pad(left: 6.4em)[
- В рамках MVP целевые платформы: Windows, Linux, macOS, Android.
- Для каждой платформы должны быть определены поддерживаемые технические способы блокировки и ограничения; ограничения должны быть явно отражены в документации и в поведении приложения.
]
...
#pad(left: 6.4em)[
- Учётные данные:
  - Google: выбор аккаунта и предоставление разрешений;
  - телефон: номер телефона и SMS-код;
  - логин/пароль: логин и пароль.
- Настройки блокировки:
  - включение/выключение категорий (браузерный ИИ / ИИ-приложения / ИИ-боты);
  - фильтры (набор правил) и уровень блокировки;
  - параметры «постепенного ужесточения»;
  - дневные лимиты (числовые значения).
- Исключения:
  - белый список (тип объекта + идентификатор);
  - временное снятие запрета по таймеру (длительность);
  - временное снятие запрета по расписанию (дни недели + интервал времени).
- Импорт/экспорт:
  - файл настроек (импорт);
  - место/формат сохранения (экспорт).
]
...
- Приложение должно отображать:
  - статус активной блокировки по категориям;
  - список активных правил (фильтров) и расписаний;
  - состояние дневных лимитов;
  - записи белого списка;
  - состояние активного таймера снятия запрета и/или таймера запрета;
  - уведомления о результате импорта/экспорта;
  - статус подписки и доступные действия.
- При ошибках (валидация, недоступность системного механизма блокировки, ошибки импорта, ошибки оплаты) приложение должно отображать понятное сообщение и не применять некорректные изменения.
]
...
#h(0em)
#pad(left: 6.4em)[
- Изменения настроек блокировки (включение категории, изменение фильтров/лимитов/исключений) должны вступать в силу сразу после сохранения и быть проверяемыми пользователем.
- Таймеры и расписания должны применяться автоматически при наступлении времени срабатывания и автоматически завершаться по истечении заданного времени.
]
