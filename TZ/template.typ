#let template(cfg: none, body) = {
    ...
    let outline_and_normal_pages = {
        ...
        show heading.where(level: 4): h => {
            ...
            block(inset: (left: 5em))[#counter(heading).display() #h.body]
        }
        show heading.where(level: 5): h => {
            set text(weight: "bold", size: 12pt)

            block(inset: (left: 7em))[#counter(heading).display() #h.body]
        }
        show heading.where(level: 6): h => {
            set text(weight: "bold", size: 12pt)

            block(inset: (left: 9em))[#counter(heading).display() #h.body]
        }
        show heading.where(level: 7): h => {
            set text(weight: "bold", size: 12pt)

            block(inset: (left: 11em))[#counter(heading).display() #h.body]
        }
